...
import {ConfPermissionDto, ConfRoleDto} from "../../model/dto";
...
export default function RoleManagement() {
    const [roles, setRoles] = useState<ConfRoleDto[]>([]);
    ...
    const [allPermissions, setAllPermissions] = useState<ConfPermissionDto[]>([]);
    const [rolePermissions, setRolePermissions] = useState<number[]>([]);
    let pageSize = 10;

    function searchForRoles() {
        const params: GetRolesRequest = {
            pageNo,
            pageSize
        };
    ...
    function onPermissionsClick(roleId: number): void {
        setCurrentRoleId(roleId);
        setAllPermissionsVisible(true);
    }

    function getAllPermissions(): void {
    ...
    function onPermissionToggled(permissionId: number, checked: boolean): void {
        if (checked) {
    ...
    return <div>
        <ThemeProvider theme={roleTableTheme}>
            ...
            <Pagination style={{float: "right", marginTop: "1rem"}} count={pages} page={pageNo}
                        onChange={(_e, page) => {setPageNo(page)}}/>
                    ...
                    <FormGroup>
                        {allPermissions.map((permission, index) => {
                            return <FormControlLabel
                                ...
                                onChange={(_e, checked) => onPermissionToggled(permission.permissionId, checked)}/>
                        })}
                    </FormGroup>
                </DialogContent>
            </ThemeProvider>
